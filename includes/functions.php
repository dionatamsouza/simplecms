<?php
	...
	function has_admin_access($username){
		// tests if the user has a admin privilages
		global $connection;
		...
		$query .= "AND user_level=1 "; // check if the user is admin
		$query .= "LIMIT 1";
		$admin_set = mysqli_query($connection, $query);
		confirm_query($admin_set);
	...
	function public_navigation($subject_array, $page_array) {
		...
		$output .= "<tr><th>Page Name</th><th>Inside Menu</th><th>Visible</th></tr>";
	...
	function get_all_img_url(){
		global $connection;
		$all_images = array();
		$query = "SELECT * FROM image_ref"; 
		$image_set = mysqli_query($connection, $query);
		confirm_query($image_set);
		while ($data = mysqli_fetch_assoc($image_set)) {
			$all_images[] = $data;
		}
		return $all_images;
	}
